...
import { FeatureGroup,  MapLayer, LeafletConsumer } from "react-leaflet";
...
class SelectVehicles extends MapLayer {
  ...
  render() {
    const zoom = this.props.leaflet.leafletElement.getZoom();
    ...
    if(!vehicles || vehicles.length === 0)
      ...
      return (
        <FeatureGroup id="vehicles fg" >
          { vehicles.map((v) => <VehicleMarker key={"vm" + v.id} zoom={zoom} vehicle={v} />) }
        </FeatureGroup>
      );
  }
}
