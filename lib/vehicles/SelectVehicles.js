...
class SelectVehicles extends MapLayer {
  ...
  isDatestampNewer(ds) {
    /** compares datestamp param */
    let retVal = true;
    if(ds && !isNaN(ds)) {
      if(this.state.lastDateStamp && this.state.lastDateStamp > ds)
        retVal = false;
      this.state.lastDateStamp = ds;
    }
    return retVal;
  }
  ...
  getVehicles() {
    ...
    const l = this.state.lastDateStamp || 1;

    fetch(`${this.props.api}/routes/${r}?time=${d}`)
    .then(res => {
      let retVal = null;
      let ds = res.headers.get('timestamp');
      if(this.isDatestampNewer(ds))
        retVal = res.json();
      return retVal;
    })
    .then(json => {
