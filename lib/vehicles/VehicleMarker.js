...
import { withLeaflet, Marker, CircleMarker, Popup, Tooltip } from "react-leaflet";
...
class VehicleMarker extends React.Component {
  ...
  makeCircleMarker(radius, weight) {
    const v = this.props.vehicle;
  ...
  makeRotatedMarker() {
    ...
    const icon = makeVehicleIcon('vehicle-marker vehicle-icon', v.routeType, v.routeShortName);
    const position = [v.lat, v.lon];
  ...
  makeMarker() {
    ...
    if(zoom >= 14)
      ...
      return this.makeCircleMarker(6.0, 2);
    else if(zoom >= 9)
      return this.makeCircleMarker(4.0, 1);
    else
      return this.makeCircleMarker(3.0, 1);
  }
