...
class VehicleGeometry extends MapLayer {
  ...
  findPointOnLine(vehicle, geom) {
    ...
    for(let i = 0; i < geom.length; i++) {
      ...
      // step 7: have we 'found' any candidate line split points from above?
      if(found > 0) {
        retVal = found;
        ...
        if(bestLat >= 0 && bestLon >= 0) {
          if(bestLat === bestLon)
            retVal = bestLat;
          else if (bestLat > bestLon)
            retVal = bestLat - Math.floor((bestLat - bestLon) / 2);
          else
            retVal = bestLon - Math.floor((bestLon - bestLat) / 2);
        }

        // step 9: we found a some split point of the line, so let's exit traversing the line points
        break;
      }
    }
